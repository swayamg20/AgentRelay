...
function validateIntentInvariant(intent: Intent, proposedAction: ProposedActionInput | null): void {
...
export async function transitionHandoff(db: Database, input: TransitionInput): Promise<Handoff> {
	return await db.transaction(async (tx) => {
		...
		switch (input.transition) {
			...
			case "cancel": {
				if (handoff.senderId !== input.callerId) {
					throw new RelayError("not_authorized_transition", "Only the sender can cancel a handoff");
				}
				if (handoff.status !== "pending") {
					throw new RelayError(
						"invalid_transition",
...
export async function getHandoff(
	...
	const [handoff] = await db.select().from(handoffs).where(eq(handoffs.id, opts.taskId));
	if (!handoff) throw new RelayError("thread_not_found", "Thread not found");
...
export async function listHandoffs(db: Database, f: ListHandoffsFilter): Promise<Handoff[]> {
	const roleClause =
		f.role === "recipient"
