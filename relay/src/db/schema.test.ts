...
import { agentCards, agents, apiKeys, auditLog, handoffs, messages } from "./schema.js";
...
d("db schema integration", () => {
	...
	it("agents: round-trips and enforces status check", async () => {
		...
		const fetched = await handle.db.select().from(agents).where(eq(agents.id, id));
	...
	it("agent_cards: round-trips with array + jsonb defaults", async () => {
		...
		const [card] = await handle.db.select().from(agentCards).where(eq(agentCards.agentId, agentId));
