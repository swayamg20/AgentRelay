...
export function createA2aRoutes(opts: A2aRoutesOptions): Hono<AppEnv> {
	...
	router.post("/", async (c) => {
		...
		if (env.jsonrpc !== "2.0" || typeof env.method !== "string") {
			return c.json(
				rpcError(id, "invalid_request", "Not a valid JSON-RPC envelope", requestId),
				400,
			);
		}
	...
	if (p.task_id) {
		...
		if (!result.replayed) {
			...
			if (parts) {
				...
				await emitNotification(ctx, "notify.message.appended", p.task_id, otherParticipant, text);
			}
		}
		return {
			// legacy
...
async function loadSenders(db: Database, ids: string[]): Promise<Map<string, SenderInfo>> {
