...
import type { CapsuleLaunchDescriptor } from "./capsule-launch-descriptor.js";
import {
...
export interface CapsuleDescriptorSelection<Descriptor extends CapsuleLaunchDescriptor> {
	readonly adapterInfo: AdapterInfo;
	resolveForInstall(directory: string, input: SessionInput): Promise<Descriptor>;
	readPersisted(directory: string): Promise<Descriptor>;
	assertCompatible(descriptor: Descriptor, input: SessionInput): void;
}

export interface PersistentCapsuleAdapterOptions<Descriptor extends CapsuleLaunchDescriptor> {
	readonly rootDirectory: string;
	readonly launcher: CapsuleLauncher;
	readonly startupTimeoutMs?: number;
	readonly selection: CapsuleDescriptorSelection<Descriptor>;
}
...
/** Shared Node-side lifecycle for independently persistent, one-Mission capsules. */
export class PersistentCapsuleAdapter<Descriptor extends CapsuleLaunchDescriptor>
	implements AgentHostAdapter, RuntimeAuthorityPort
{
	readonly #rootDirectory: string;
	...
	readonly #descriptorReadiness = new Map<string, Promise<Descriptor>>();
	...
	static async open<Descriptor extends CapsuleLaunchDescriptor>(
		options: PersistentCapsuleAdapterOptions<Descriptor>,
	): Promise<PersistentCapsuleAdapter<Descriptor>> {
		...
		await ensurePersistentCapsuleDirectory(rootDirectory);
		const decoded = await readPersistentCapsuleJsonIfPresent(
			join(rootDirectory, CAPSULE_REGISTRY_FILE),
		);
		const registry = capsuleRegistrySchema.parse(decoded ?? { schema_version: 1, executions: {} });
		validateRegistry(registry);
		return new PersistentCapsuleAdapter(
			{
				rootDirectory,
	...
	async probe(): Promise<AdapterInfo> {
		return structuredClone(this.#selection.adapterInfo);
	}
	...
	async terminateAll(): Promise<void> {
		...
		for (const entry of entries) {
			...
			try {
				descriptor = await this.#selection.readPersisted(join(this.#rootDirectory, entry.name));
				assertDescriptorMissionScope(descriptor, entry.name);
			} catch (error) {
				failures.push(error);
				continue;
			}
	...
	private async ensureDescriptor(input: SessionInput): Promise<Descriptor> {
		let readiness = this.#descriptorReadiness.get(input.missionId);
		if (readiness === undefined) {
			readiness = this.#selection.resolveForInstall(this.capsuleDirectory(input.missionId), input);
			this.#descriptorReadiness.set(input.missionId, readiness);
		}
		try {
			const descriptor = await readiness;
			this.#selection.assertCompatible(descriptor, input);
			return descriptor;
		} finally {
	...
	private async requireDescriptor(
		...
		const descriptor = await this.#selection
			.readPersisted(this.capsuleDirectory(missionId))
			.catch((error) => {
				if (allowMissing && isMissingCapsuleDescriptor(error))
					throw new CapsuleRpcError("transport", "Capsule session is not initialized", {
						cause: error,
					});
				throw error;
			});
		assertDescriptorMissionScope(descriptor, missionId);
		...
		this.#selection.assertCompatible(descriptor, descriptor.session);
		return descriptor;
	}
	...
	private ensureCapsule(descriptor: Descriptor, requireActiveAuthority = true): Promise<void> {
	...
	private async ensureCapsuleReady(descriptor: Descriptor): Promise<void> {
	...
	private async startOrAwaitCapsule(descriptor: Descriptor): Promise<void> {
	...
	private async prepareCapsuleLaunch(descriptor: Descriptor): Promise<CapsuleLaunchPreparation> {
		try {
			await this.probeCapsule(descriptor);
			return "ready";
		} catch (error) {
	...
	private async probeCapsule(descriptor: Descriptor): Promise<void> {
		...
		if (!isDeepStrictEqual(info, this.#selection.adapterInfo)) {
			throw new CapsuleRpcError(
				"correlation_conflict",
	...
	private async syncAuthorityWithinTransition(descriptor: Descriptor): Promise<void> {
	...
	private async shutdownUninstalledCapsule(descriptor: Descriptor): Promise<void> {
		try {
	...
	private async waitForCapsuleRetirement(descriptor: Descriptor): Promise<void> {
		const deadline = Date.now() + this.#startupTimeoutMs;
		let waitMs = 10;
		while (Date.now() < deadline) {
			try {
...
export async function ensurePersistentCapsuleDirectory(directory: string): Promise<void> {
...
export async function readPersistentCapsuleJsonIfPresent(path: string): Promise<unknown | null> {
...
function assertDescriptorMissionScope(
	descriptor: CapsuleLaunchDescriptor,
	missionId: string,
): void {
	if (descriptor.session.missionId !== missionId) {
		throw new CapsuleRpcError(
			"scope_mismatch",
			"Mission capsule descriptor is stored under a different Mission directory",
		);
	}
}
