...
type RelayChild = ChildProcessWithoutNullStreams | ChildProcessByStdio<null, Readable, Readable>;
	...
	async waitForHealth(timeoutMs = 15_000): Promise<void> {
		...
		while (Date.now() < deadline) {
			if (this.exit !== undefined) {
				throw new Error(`relay exited before health check passed (${formatExit(this.exit)})`);
			}
